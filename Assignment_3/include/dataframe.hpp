...
class Dataframe{
    public:
        ...
        void printCovMat(const std::vector<std::string>& attributes) const;
        // Method to print the correlation Matrix
        void printCorrMat(const std::vector<std::string>& attributes) const;
        ...
        //--------------------------------------------------------------------------------------------
        // These were added for Homework 3
        // Method to set a value to a df existing entry
        void setDfEntry(const std::pair<unsigned int, std::string> &idx, const ColumnValue &value);
        // Method to update an entire existing column
        void setDfColumn(const std::string &attribute, const std::vector<ColumnValue> &column);
        // Method to update an entire existing row
        void setDfRow(const unsigned int &idx, const std::vector<ColumnValue> &row);
        // Method to set dimensions private values (wrapper method)
        void preallocDims(const unsigned int &nrows,  const unsigned int &ncols);
        //--------------------------------------------------------------------------------------------
    private:
        //--------------------------------------------------------------------------------------------
        // Helper methods
